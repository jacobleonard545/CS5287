...
messages_processed_success = Counter(
    'processor_messages_processed_success_total',
    'Total number of successfully processed messages',
    ['topic']
)
messages_processed_failed = Counter(
    'processor_messages_processed_failed_total',
    'Total number of failed message processing attempts',
    ['topic']
)
influxdb_writes_total = Counter(
    'processor_influxdb_writes_total',
    'Total number of writes to InfluxDB',
    ['bucket']
)
influxdb_write_errors = Counter(
    'processor_influxdb_write_errors_total',
    'Total number of InfluxDB write errors',
    ['bucket']
)
processing_duration_seconds = Histogram(
    'processor_message_processing_duration_seconds',
...
current_speed_gauge = Gauge(
    'processor_current_speed_ms',
    'Current conveyor speed being processed (m/s)',
    ['conveyor_id']
)
...
class ConveyorDataProcessor:
    ...
    def detect_anomalies(self, current_speed, current_state):
        ...
        if len(self.rolling_window) >= 2:
            ...
            if speed_change > 0.1:
                ...
                anomalies_detected_total.labels(anomaly_type="rapid_change").inc()
    ...
    def process_message(self, message):
        ...
        try:
            ...
            current_speed_gauge.labels(conveyor_id=conveyor_id).set(speed_ms)
            ...
            # Increment InfluxDB write counter
            influxdb_writes_total.labels(bucket=INFLUXDB_BUCKET).inc()
            ...
            messages_processed_success.labels(topic=KAFKA_TOPIC).inc()
            ...
            messages_processed_failed.labels(topic=KAFKA_TOPIC).inc()
            if "influx" in str(e).lower():
                influxdb_write_errors.labels(bucket=INFLUXDB_BUCKET).inc()
    ...
    def run(self):
        """Main processor loop"""
        logger.info("Starting Conveyor Data Processor for CA3...")
