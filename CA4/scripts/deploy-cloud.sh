...
# Get cloud instance IP from Terraform
CLOUD_IP=$(cd CA4/terraform && ~/bin/terraform.exe output -raw cloud_instance_public_ip)
...
scp -i "$SSH_KEY" -o StrictHostKeyChecking=no ../docker-compose-cloud.yml ubuntu@$CLOUD_IP:~/docker-compose.yml
scp -i "$SSH_KEY" -o StrictHostKeyChecking=no ../.env.cloud ubuntu@$CLOUD_IP:~/.env
...
# Build processor image on cloud (or pull if already pushed)
...
# Copy processor code
scp -i "$SSH_KEY" -o StrictHostKeyChecking=no ../../docker/processor/conveyor_processor.py ubuntu@$CLOUD_IP:~/processor/
...
echo ""
echo "Next: Run './scripts/setup-ssh-tunnels.sh' to establish connectivity"
